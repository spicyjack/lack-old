...
## FUNC: check_filelist_envvar - Check the $FILELIST environment variable
## DESC: exits the script with an error if the filelist variable is not set
...
## FUNC: check_project_list_envvar - Check for the $PROJECT_LIST variable
## DESC: exits the script with an error if the project list variable is not set
...
## FUNC: check_base_file_exists - Check for the $PROJECT_NAME.base.txt
## DESC: Exits the script with an error if the project base file is missing.
...
## FUNC: create_initramfs_filelist - Creates the initramfs filelist
## DESC: Writes the initramfs filelist to the $TEMP_DIR
## DESC: Requires that the $PROJECT_DIR and $PROJECT_NAME environment
## DESC: variables be set prior to calling this function
...
## FUNC: create_hostname_file - Create the hostname file in $TEMP_DIR
function create_hostname_file {
    echo $PROJECT_NAME > $TEMP_DIR/hostname
} # function check_filelist_envvar

## FUNC: create_standalone_init_script - Create the /init script
...
## DESC: Performs a bunch of variable substitutions when it copies the file to
## DESC: $TEMP_DIR
...
## DESC: process.  Performs a bunch of variable substitutions when it copies
## DESC: the file to $TEMP_DIR.
function create_initramfs_init_script {
    _create_init_script _initramfs_init.sh
} # function create_initramfs_init_script

## FUNC: _create_standalone_init_script - Actually create the /init script
...
## FUNC: copy_ssl_pems - Copy the *.pem files needed for SSL usage
## DESC: Exits if it can't find the correct files in the ~/stuff_tars
## DESC: directory.  Requires that $PROJECT_NAME and $TEMP_DIR be set
...
## FUNC: copy_busybox_binary - Copies the Busybox binary to $TEMP_DIR
## DESC: Exits if it can't find the Busybox binary.  Requires $TEMP_DIR be set
